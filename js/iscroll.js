...
iScroll.prototype = {
	...
	_scrollbar: function (dir) {
		...
		if (!that[dir + 'ScrollbarWrapper']) {
			...
			else bar.style.cssText = 'position:absolute;z-index:10;' + (dir == 'h' ? 'height:7px;bottom:1px;left:2px;right:' + (that.vScrollbar ? '7' : '2') + 'px' : 'width:7px;bottom:' + (that.hScrollbar ? '7' : '2') + 'px;top:2px;right:1px');
			...
			if (!that.options.scrollbarClass) {
				bar.style.cssText = 'position:absolute;z-index:10;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.9);-' + vendor + '-background-clip:padding-box;-' + vendor + '-box-sizing:border-box;' + (dir == 'h' ? 'height:100%' : 'width:100%') + ';-' + vendor + '-border-radius:3px;border-radius:3px';
			}
...
//			else if (scale > that.options.scaleMax) scale = 2.0 * that.options.scaleMax * Math.pow(0.5, that.options.scaleMax / scale);
	...
	_bind: function (type, el, bubble) {
		if(vendor == '' && parseInt($.browser.version) < 9){
		}else{
